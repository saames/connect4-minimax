...
def avaliar_janela(janela, peca):
    pontuacao = 0
    peca_oponente = JOGADOR if peca == IA else IA

    qtd_peca = janela.count(peca)
    qtd_vazio = janela.count(VAZIO)
    qtd_oponente = janela.count(peca_oponente)

    # Ataque
    if qtd_peca == 3 and qtd_vazio == 1:
        pontuacao += 15
    elif qtd_peca == 2 and qtd_vazio == 2:
        pontuacao += 5

    # Defesa
    if qtd_oponente == 3 and qtd_vazio == 1:
        pontuacao -= 70
    elif qtd_oponente == 2 and qtd_vazio == 2:
        pontuacao -= 2

    return pontuacao

def pontuacao_posicao(tabuleiro, peca):
    pontuacao = 0

    # Bônus da Coluna Central
    coluna_central = [tabuleiro[r][COLUNAS // 2] for r in range(LINHAS)]
    pontuacao += coluna_central.count(peca) * 3
    # Horizontais
    for r in range(LINHAS):
        linha_array = tabuleiro[r]
        for c in range(COLUNAS - 3):
            janela = linha_array[c:c+4]
            pontuacao += avaliar_janela(janela, peca)
    # Verticais
    for c in range(COLUNAS):
        coluna_array = [tabuleiro[r][c] for r in range(LINHAS)]
        for r in range(LINHAS - 3):
    ...
    for r in range(LINHAS - 3):
        for c in range(COLUNAS - 3):
            janela = [tabuleiro[r+3-i][c+i] for i in range(4)]
            pontuacao += avaliar_janela(janela, peca)
